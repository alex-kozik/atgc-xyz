...
def simulation(numPoolSize):
	...
	avg = round(sum(fetchedList)/len(fetchedList),4) # average
	mi = min(fetchedList)
	ma = max(fetchedList)
	...
	k = 0
	while k <= 300:
		count_balls = fetchedList.count(k)
		print "++ " + '\t' + `numPoolSize` + '\t' + `k` + '\t' + `count_balls` + '\t' + " ++"
		k = k + 1
	percent_median = round((median/300.0),4)
	...
	print "== " + '\t' + `numPoolSize` + '\t' + str(avg) + '\t' + `median`  + '\t' + `mi` + '\t' + `ma` + '\t' + `effectiveMin` + '\t' + `balls300` + '\t' + str(percent_median) + '\t' + " =="
