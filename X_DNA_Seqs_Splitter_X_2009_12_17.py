...
def Seqs_Cleaner(in_name, out_name, seq_type, trans_fr, gen_code):
	...
	out_fileX = open(out_name + '.XXX.fasta', "wb")
	...
	while 1:
		...
		if t == '':
			...
			if have_seqs != "":
				...
				##########################
				###    SPLIT BY XXX    ###
				my_subsetX = have_seqs.split('X')
				fragment_n = 1
				for fragment in my_subsetX:
					fragment_len = len(fragment)
					if fragment_len > 60:
						out_fileX.write('>' + proper_id + '_' + `fragment_n` + ' [ FR_LEN: ' + `fragment_len` + ' ] ' + good_name + '\n')
						out_fileX.write(fragment + '\n')
						fragment_n = fragment_n + 1
				##########################
				break
		if '\n' in t:
			t = t[:-1]
...
\n\n  Ooops... ID duplication  \n\n  check input for duplications  \n\n\n ID: " + good_head + "\n\n\n"
			...
			if line_counter != 1:
				...
				###
				###  BINARY ###
				# out_bin.write(proper_id + " " + `seqs_len` + '\n')
				# bin_list = list(have_seqs)
				# for bin_item in bin_list:
				#	if bin_item in agct_list:
				#		if bin_item == "A":
				#			out_bin.write("00")
				#		if bin_item == "G":
				#			out_bin.write("01")
...
if __name__ == "__main__":
	...
	if len(sys.argv) <= 5 or len(sys.argv) > 6:
		print ""
		print "  Program usage: "
		print "  input_file output_file DNA/prot trans_frame[0 1 3 6] genetic_code[1 2 3]"
	...
	if len(sys.argv) == 6:
		...
		if gen_code != 1 and gen_code != 2 and gen_code != 3:
			...
			sys.exit()
		if in_name != out_name:
			Seqs_Cleaner(in_name, out_name, seq_type, trans_fr, gen_code)
		else:
