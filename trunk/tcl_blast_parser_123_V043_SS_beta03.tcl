...
##############################################################
	...
	set f_query [open [lindex $argv 6] "r"]
	...
	global q_seq_array_str
	global q_seq_array_len

	file delete -force "x-groups-for-alignment"
	file mkdir "x-groups-for-alignment"

	set q_seq_count 0
	while {[gets $f_query q_seq_line] >= 0} {
		set q_seq_id   [lindex [split $q_seq_line] 0]
		set q_seq_len  [lindex [split $q_seq_line] 1]
		set q_seq_str  [lindex [split $q_seq_line] 2]
		set q_seq_array_str($q_seq_id) $q_seq_str
		set q_seq_array_len($q_seq_id) $q_seq_len
		incr q_seq_count
		puts $q_seq_count
	}
	...
	while {[gets $f_in t] >= 0} {
		...
		### OPEN FILE FOR ALIGNMENT ###
		set sub_align_open [open "x-groups-for-alignment\/$query_id\.4aln" "w"]
		puts -nonewline $sub_align_open ">"
		puts -nonewline $sub_align_open "$query_id\+"
		puts $sub_align_open "  = QUERY_SEQUENCE =  LENGTH: \[ $q_seq_array_len($query_id) \] "
		puts $sub_align_open $q_seq_array_str($query_id)
		###############################
		...
		set q_subject_dir [string range $str 2 3]
		set q_query_dir   [string range $str 0 1]
	    }
		...
		######################
		if {$alt_align_count == 1} {
		    set q_subject_str $s_align
		    regsub -all {\-} $q_subject_str "" q_subject_str
		    set q_subject_str [string toupper $q_subject_str]
		    set q_subject_len [string length $q_subject_str]
		    set q_subject_diff [expr $s_len - $q_subject_len]
		    puts $f_out10 "$qs\t$qe\t$ss\t$se\t$q_len\/$s_len\t$gap_info"
		    puts $sub_align_open "\>$subj$q_subject_dir  = SUBJECT_SEQUENCE =  LENGTH: \[ $q_subject_len \] FRAGMENT: \[ $ss-$se \] OF \[ $s_len \] ON $query_id \[ $qs-$qe \] CLIP: \[ $q_subject_diff \] IDENTITY: \[ $idn \] "
		    puts $sub_align_open $q_subject_str
		}
		...
		close $sub_align_open
		file delete -force "x-groups-for-alignment\/$query_id\.4aln"
	    }
...
if {$argc != 7} {
    ...
    puts "Program usage:"
    puts "tcl_blast_parser_123.tcl \[input\], \[output\], \[Exp(20)\], \[Idnt(40)\], \[Overlap(100)\], \[MATRIX/GRAPH option\], \[TAB-Query-Seqs\]"
