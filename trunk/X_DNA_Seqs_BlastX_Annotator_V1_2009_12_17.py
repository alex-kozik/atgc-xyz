...
def Seqs_Drobilka(in_name, out_name, seq_type, group_f, group_id):
	...
	annotation_array = {}
	annotation_status = {}
	annotation_string_array = {}

	print_frame = "FALSE"
	### TRY TO READ GROUP LIST ###
	try:
		frame_file = open(group_f, "rb")
		...
		while 1:
			...
			bit_score = ""
			identity = ""
			annotation_confidence = " NO_SIMILARITY_FOUND "
			annotation_string = annotation_confidence + " =[ NOT_ANNOTATED ]= "
			if len(u) >= 8:
				hit_id = u[1]
				annotation = u[2]
				expect = u[3]
				...
				if expect_value >= 20:
					annotation_confidence = " STRONG SIMILARITY TO: "
				if expect_value < 20:
					annotation_confidence = " WEAK SIMILARITY TO: "
				bit_score = u[4]
				identity = u[5]
				print_annotation = "TRUE"
				annotation_string = annotation_confidence + ' [ ' + hit_id + ' ANNOTATION: ' + annotation + ' ] ' \
							+ ' EXPECT: ' + expect + ' IDENTITY: ' + identity
			frame_array[fl] = fl
			hit_id_array[fl] = hit_id
			annotation_array[fl] = annotation
			expect_array[fl] = expect
			identity_array[fl] = identity
			annotation_status[fl] = print_annotation
			annotation_string_array[fl] = annotation_string
			print_frame = "TRUE"
	except:
		print "DID NOT FIND GROUP FILE:  " + group_f
		print "WORKING WITHOUT GROUP LIST"

	if print_frame == "TRUE":
		out_group  = open(out_name + '.subgroup' + '_' + group_id, "wb")
			...
			if have_seqs != "":
				...
				if print_frame == "TRUE":
					try:
						...
						annotation_string = annotation_string_array[proper_id]
						# out_group.write('>' + proper_id + ' ' + good_name + ' ' + annotation_string + '\n')
						out_group.write('>' + proper_id + ' ' + annotation_string + '\n')
						out_group.write(have_seqs + '\n')
						print proper_id + " -=- EXTRACTED -=-"
					except:
						print proper_id + " WAS NOT EXTRACTED"
				##########################
				break
		if '\n' in t:
			t = t[:-1]
...
if __name__ == "__main__":
	if len(sys.argv) <= 5 or len(sys.argv) > 6:
		print ""
		print "Program usage: "
		print "[input_file] [output_file] [DNA/prot] [group_file] [group_id]"
		print "Script counts \"ATGC\" content in FASTA file"
		print "and generates tab-delimited file for mySQL db"
		print ""
		print "if group file is provided FASTA file for that group will be generated"
		print ""
		exit
	if len(sys.argv) == 6:
		in_name  = sys.argv[1]
		...
		group_id = sys.argv[5]
		if in_name != out_name:
			Seqs_Drobilka(in_name, out_name, seq_type, group_f, group_id)
		else:
